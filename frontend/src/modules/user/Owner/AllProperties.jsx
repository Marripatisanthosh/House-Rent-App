...
const AllProperties = () => {
   ...
   const getAllProperty = async () => {
      try {
         const response = await axios.get('https://house-rent-self.vercel.app/api/owner/getallproperties', {
   ...
   const saveChanges = async (propertyId, status) => {
      try {
         ...
         const res = await axios.patch(`https://house-rent-self.vercel.app/api/owner/updateproperty/${propertyId}`, formData, {
            headers: { 'Authorization': `Bearer ${localStorage.getItem("token")}` }
         })
         if (res.data.success) {
   ...
   const handleDelete = async (propertyId) => {
      ...
      if (assure) {
         try {
            const response = await axios.delete(`https://house-rent-self.vercel.app/api/owner/deleteproperty/${propertyId}`, {
               headers: { 'Authorization': `Bearer ${localStorage.getItem("token")}` }
            });
               ...
               <TableBody>
                  {allProperties.map((property) => (
                     ...
                     >
                        ...
                        {/* <TableCell align="center"><img style={{width: 150, height: 150}} src={`https://house-rent-self.vercel.app${property.propertyImage.path}`} alt="photos" /></TableCell> */}
